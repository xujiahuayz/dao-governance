...
for _, row in tqdm(df_proposals.iterrows(), total=len(df_proposals)):
    ...
    if len(df_vote_subset["type"].unique()) == 1:
        df_vote_subset = process_vote(df_vote_subset)
    ...
    # Split voters into whales and non-whales
    voting, unknown_voters = voter_split(
        df_vote_subset,
        total_holding_data,
        total_holding,
        start_ts,
    ...
    non_whale_vote = {k: v for k, v in voting.items() if v["label"] == "non_whales"}
    non_whale_vote = dict(
        sorted(non_whale_vote.items(), key=lambda item: item[1]["vp"])
    )
    whale_vote = {k: v for k, v in voting.items() if v["label"] == "whales"}
    ...
    non_whale_freq = calc_frequency([v["choice"] for k, v in non_whale_vote.items()])
    whale_vp = calc_vp(
    ...
    non_whale_votable_contract_no_delegate = deepcopy(non_whale_votable_contract_set)
    ...
    for group in ["whale", "non_whale"]:
        ...
        if delegation == "delegation":
            proposal_participation[f"{group}_participation"] = (
                (
                    proposal_participation[f"{group}_vote_num"]
                    + proposal_participation[f"{group}_from_delegation_num"]
                )
                / proposal_participation[f"{group}_num"]
                if proposal_participation[f"{group}_num"] > 0
                else np.nan
    ...
    non_whale_num = len(total_non_whale_set | non_whale_votable_contract_no_delegate)
    proposal_participation["holder_num"] = holder_num
    proposal_participation["whale_num"] = whale_num
    proposal_participation["non_whale_num"] = non_whale_num
...
for group in ["non_whale", "whale"]:
    for metric in ["num", "vote_num", "turnout"]:
        df_proposals_participation[f"{group}_{metric}_delegate"] = (
